...
def acquire_images(device, stream, output_filename_prefix="hyperspectral_data"):
        ...
        if result.IsOK():
            if operational_result.IsOK():
                ...
                if cv_pixel_type == "MONO12PACKED":
                    ...
                    ### 240610 수정 (added)
                    collected_images.append(data) # ushort 변환된 데이터를 추가
                   
                     # 리쉐이핑하여 waterfall에 맞게 (240610 추가 후 다시 제거)
                    # reshaped_data = np.sum(data, axis=0).reshape((1, width)).astype(np.uint8)  # [수정됨]
                    # reshaped_data_color = cv2.cvtColor(np.expand_dims(reshaped_data, axis=2), cv2.COLOR_GRAY2RGB)
                    ...
                    ## 240610 수정 (주석처리, .npy 저장 제대로 안됨)
                    # collected_images.append(merged_image)  # Collect images

                    ## 240610 수정 (merged imagse-> reshaped_data_color) 후 다시 수정(원래대로)
                    waterfall_image = waterfall_update(waterfall_image, merged_image)
                    cv2.imshow("Waterfall", waterfall_image)
                    # waterfall_image = waterfall_update(waterfall_image, reshaped_data_color)
                    # cv2.imshow("Waterfall", waterfall_image)
    ...
    np.save(f"outputs/{output_filename_prefix}_{timestamp}_data.npy", np.array(collected_images))
    print(f"Saved data to outputs/{output_filename_prefix}_{timestamp}_data.npy")
